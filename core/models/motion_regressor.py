...
class MotionTokenTransformer(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        self.style_cond_emb_dim  = style_cond_dim
        ...
        if exists(self.cond_emb_dim):
      
            self.project_cond_emb = nn.Linear(self.cond_emb_dim, self.emb_dim) if self.cond_emb_dim != self.emb_dim else nn.Identity()
        
        if exists(self.style_cond_emb_dim):
            
      
            self.project_style_cond_emb = nn.Linear(self.style_cond_emb_dim, self.emb_dim) if self.style_cond_emb_dim != self.emb_dim else nn.Identity()
...
class MotionRegressorModel(nn.Module):
    ...
    def __init__(self, args, pad_value = 0, device = "cuda"):
    ...
    def generate(
        ...
        for sl in tqdm(range(seq_len) ,  position=0, leave=True):
            ...
            logits = self.forward(motion = x, context = context[:,:(sl+1)] , context_mask = context_mask[:,:(sl+1)] , style_context = style_context )[:, -1]
    ...
    def forward(self, motion , mask = None , context = None,context_mask = None,self_attn_context_mask= None , style_context = None):
